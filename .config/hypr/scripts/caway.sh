...
FORMAT='{"text": "{{artist}} - {{title}}", "tooltip": "{{playerName}} : {{markup_escape(artist)}} - {{markup_escape(title)}}", "alt": "{{status}}", "class": "{{status}}"}'

# playerctl output into playerctl_waybar_pipe
playerctl -a metadata --format "$FORMAT" -F >$playerctl_waybar_pipe &
...
while read -r line; do
    ...
    # if status is stopped then return empty
    if [[ $status == 'Stopped' ]]; then
        echo '{"text": "", "tooltip": "", "alt": "", "class": ""}' | jq --unbuffered --compact-output
        continue
    fi

    # If the class says "Playing" and equilizer is enabled
    # then show the cava equilizer
    if [[ $EQUILIZER == 1 && $status == 'Playing' ]]; then
        ...
        while read -r cmd2; do
            # If the cmd2 is empty then return empty
            if [[ -z $cmd2 ]]; then
                echo '{"text": "", "tooltip": "", "alt": "", "class": ""}' | jq --unbuffered --compact-output
                continue
            fi
